...
RDKS100 系统是面向大型商超仓库、家具零售仓库、仓储园区和室内物流中转区的智能巡检管控系统。作品以 RDKS100 边缘计算平台为上位机，以STM32F103 为底盘控制下位机，结合阿克曼转向移动底盘、LivoxMid-360S三维激光雷达、IntelRealSenseD435iRGB-D 相机、YOLOv5 目标检测、VLM 视觉语言理解、CartographerSLAM 与Nav2导航，构建了“自主巡检、异常识别、远程管控、安全停车、事件留痕”的仓储巡检闭环。 
仓储环境具有货架密集、通道狭长、人员与车辆混行、纸箱木板等可燃物集中、夜间值守成本高、异常发现依赖人工经验等特点。RDKS100 系统将移动机器人作为仓储现场的流动感知节点，可按预设路线巡检主通道、货架端头、装卸区、消防通道和高风险堆放区；通过RGB-D 图像与VLM语义理解识别火/烟、通道占用、人员靠近、障碍物遮挡和现场异常描述；通过 Web 上位机向管理人员展示视频、点云、地图、路径、告警和设备状态。 
系统突出四项创新：本地轻量化VLM在公网断开时仍能联动Nav2完成识别、绕行和告警；小脑/中核/大脑异构协同把硬实时控制、机器人服务和AI推理分层部署；2DSLAM 地图可转换为前端可编辑栅格语义地图；操作人员可通过网页或语音输入自然语言任务，由 VLM 结合当前画面与地图语义拆解并调用Nav2 分段执行。
